...
const write_data_medias = async (file_name, data, source_media) => {
  ...
  return axios({
    method: "POST",
    url: `http://search-inside-elastic.wwp-test.svc.cluster.local:9200/_doc?pipeline=attachment`,
    data: {
      url: `${source_media}`,
      rights: `test`,
      data: `${data}`,
...
const write_data_pages = async (link_page, title_page, StripHTMLBreakLines) => {
  ...
  return axios({
    method: "POST",
    url: `http://search-inside-elastic.wwp-test.svc.cluster.local:9200/_doc`,
    data: {
      url: `${link_page}`,
      title: `${title_page}`,
...
const delete_inside_temp = async () => {
  ...
  return axios({
    method: "DELETE",
    url: `http://search-inside-elastic.wwp-test.svc.cluster.local:9200`,
  });
};
...
const create_attachment_field = async () => {
  ...
  return axios({
    method: "PUT",
    url: `http://search-inside-elastic.wwp-test.svc.cluster.local:9200/_ingest/pipeline/attachment`,
    data: {
...
const copy_inside_temp_to_inside = async () => {
  ...
  return axios({
    method: "POST",
    url: `http://search-inside-elastic.wwp-test.svc.cluster.local:9200/_reindex`,
    data: {
      source: {
        index: "inside_temp",
      },
